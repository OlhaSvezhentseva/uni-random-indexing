...
def print_results(result):
    ...
    # Tipp: Der folgende, auskommentierte Code erzeugt einen Grafik mit
    #       matplotlib. Dafür muss man das entsprechende import-Statement
    #       oben aktivieren.
...
def wrap_base_dir(filename, base_dir):
...
def classify_and_evaluate(train, labels, documents, gold, base_dir: Optional[str] = "random_indexing"):
    train_docs = read_from_file(wrap_base_dir(train, base_dir))
    train_classes = read_from_file(wrap_base_dir(labels, base_dir))
    docs = read_from_file(wrap_base_dir(documents, base_dir))
    classes = read_from_file(wrap_base_dir(gold, base_dir))
